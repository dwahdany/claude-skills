...
$S receipts/get/123                                     # by-id endpoints: id in the path (verified; spec writes .../id_by_customer)
...
- Split `amounts` are **positive absolute values** even for outgoing (negative) transactions (verified live: `"95.05"` posted against a `-95.05` transaction); BB stores every posting amount unsigned and carries the direction in `credit_type` S/H. BB appends the transaction's counterparty to your `postingtext` ("… - Hetzner Online Gmbh"), so keep texts short. Belegmatching after `receipts/upload` is asynchronous: check `transactions/assigned-receipts/get` and assign explicitly if the receipt is not linked yet.
- Ids: requests send integers; responses return them as strings or integers depending on the endpoint — cast before comparing. `id_by_customer` is the number shown in the BB UI. By-id endpoints take the id as the last path segment (`receipts/get/123`, `transactions/get/123`, `receipts/delete/123`, `receipts/restore/123`; verified live) and return one object in `data`.
